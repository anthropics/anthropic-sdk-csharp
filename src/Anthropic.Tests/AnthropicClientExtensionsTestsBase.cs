...
{
    protected abstract IChatClient CreateChatClient(AnthropicClient client, string? modelId = null);
    ...
    protected IChatClient CreateChatClient(VerbatimHttpHandler handler, string? modelId = null) =>
        CreateChatClient(CreateAnthropicClient(handler), modelId);
    ...
    public async Task GetResponseAsync_WithWebSearchCitationsInTextBlock()
    {
        ...
        Assert.NotEmpty(citation.AnnotatedRegions);
        var region = citation.AnnotatedRegions.OfType<TextSpanAnnotatedRegion>().FirstOrDefault();
        Assert.NotNull(region);
        Assert.Null(region.StartIndex);
        Assert.Null(region.EndIndex);
    }
    ...
    public async Task GetResponseAsync_WithContentBlockLocationCitationsInTextBlock()
    {
        ...
        var region = citation.AnnotatedRegions?.OfType<TextSpanAnnotatedRegion>().FirstOrDefault();
        Assert.NotNull(region);
        Assert.Equal(15, region.StartIndex);
        Assert.Equal(45, region.EndIndex);
    }
