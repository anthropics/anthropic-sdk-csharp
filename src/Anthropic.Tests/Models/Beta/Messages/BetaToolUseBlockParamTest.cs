...
public class BetaToolUseBlockParamCallerTest : TestBase
{
    ...
    {
        BetaToolUseBlockParamCaller value = new(
        ...
        BetaToolUseBlockParamCaller emptyValue = new(JsonSerializer.Deserialize<JsonElement>("{}"));

        Assert.Throws<AnthropicInvalidDataException>(() => emptyValue.Type);
        Assert.Null(emptyValue.ToolID);

        BetaToolUseBlockParamCaller mismatchedValue = new(
            JsonSerializer.Deserialize<JsonElement>(
                """
                {
                  "tool_id": [
