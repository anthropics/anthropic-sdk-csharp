...
/// <summary>
/// The caller's organization or workspace cannot make model requests — out of credits
/// or spend limit reached. Retrying with the same credentials will not succeed; the
/// caller must resolve the billing state.
/// </summary>
[JsonConverter(
    typeof(JsonModelConverter<BetaManagedAgentsBillingError, BetaManagedAgentsBillingErrorFromRaw>)
)]
public sealed record class BetaManagedAgentsBillingError : JsonModel
{
    /// <summary>
    ...
    public required RetryStatus RetryStatus
    {
        ...
        {
            this._rawData.Freeze();
            return this._rawData.GetNotNullClass<RetryStatus>("retry_status");
        }
        init { this._rawData.Set("retry_status", value); }
    }

    public required ApiEnum<string, BetaManagedAgentsBillingErrorType> Type
    {
        get
        {
            this._rawData.Freeze();
...
#pragma warning disable CS8618
    [SetsRequiredMembers]
    public BetaManagedAgentsBillingError(
        BetaManagedAgentsBillingError betaManagedAgentsBillingError
    )
        : base(betaManagedAgentsBillingError) { }
#pragma warning restore CS8618

    public BetaManagedAgentsBillingError(IReadOnlyDictionary<string, JsonElement> rawData)
    {
...
#pragma warning disable CS8618
    [SetsRequiredMembers]
    BetaManagedAgentsBillingError(FrozenDictionary<string, JsonElement> rawData)
    {
        this._rawData = new(rawData);
    }
#pragma warning restore CS8618

    /// <inheritdoc cref="BetaManagedAgentsBillingErrorFromRaw.FromRawUnchecked"/>
    public static BetaManagedAgentsBillingError FromRawUnchecked(
...
class BetaManagedAgentsBillingErrorFromRaw : IFromRawJson<BetaManagedAgentsBillingError>
{
    /// <inheritdoc/>
    public BetaManagedAgentsBillingError FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawData
    ) => BetaManagedAgentsBillingError.FromRawUnchecked(rawData);
}
...
public record class RetryStatus : ModelBase
{
    ...
    public RetryStatus(BetaManagedAgentsRetryStatusRetrying value, JsonElement? element = null)
    {
        this.Value = value;
        this._element = element;
    }

    public RetryStatus(BetaManagedAgentsRetryStatusExhausted value, JsonElement? element = null)
    {
        this.Value = value;
        this._element = element;
    }

    public RetryStatus(BetaManagedAgentsRetryStatusTerminal value, JsonElement? element = null)
    {
        this.Value = value;
        this._element = element;
    }

    public RetryStatus(JsonElement element)
    {
        this._element = element;
    }
        ...
        switch (this.Value)
        {
            ...
            default:
                throw new AnthropicInvalidDataException(
                    "Data did not match any variant of RetryStatus"
                );
        }
    }
    ...
    public static implicit operator RetryStatus(BetaManagedAgentsRetryStatusRetrying value) =>
        new(value);

    public static implicit operator RetryStatus(BetaManagedAgentsRetryStatusExhausted value) =>
        new(value);

    public static implicit operator RetryStatus(BetaManagedAgentsRetryStatusTerminal value) =>
        new(value);
    ...
    public virtual bool Equals(RetryStatus? other) =>
        other != null
...
sealed class RetryStatusConverter : JsonConverter<RetryStatus>
{
    public override RetryStatus? Read(
    ...
    )
    {
        ...
        switch (type)
        {
            ...
            default:
            {
                return new RetryStatus(element);
            }
        }
    }
...
[JsonConverter(typeof(BetaManagedAgentsBillingErrorTypeConverter))]
public enum BetaManagedAgentsBillingErrorType
{
    BillingError,
}

sealed class BetaManagedAgentsBillingErrorTypeConverter
    : JsonConverter<BetaManagedAgentsBillingErrorType>
{
    public override BetaManagedAgentsBillingErrorType Read(
        ref Utf8JsonReader reader,
        System::Type typeToConvert,
        JsonSerializerOptions options
    )
    {
        return JsonSerializer.Deserialize<string>(ref reader, options) switch
        {
            ...
            _ => (BetaManagedAgentsBillingErrorType)(-1),
        };
    }

    public override void Write(
        Utf8JsonWriter writer,
        BetaManagedAgentsBillingErrorType value,
        JsonSerializerOptions options
    )
    {
        JsonSerializer.Serialize(
