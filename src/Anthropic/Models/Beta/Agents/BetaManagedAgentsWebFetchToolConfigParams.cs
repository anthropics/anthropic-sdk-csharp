...
[JsonConverter(
    typeof(JsonModelConverter<
        ...
        BetaManagedAgentsWebFetchToolConfigParamsFromRaw
    >)
)]
public sealed record class BetaManagedAgentsWebFetchToolConfigParams : JsonModel
{
    /// <summary>
    /// Must be "web_fetch".
    /// </summary>
    ...
    /// Each entry is a plain hostname like "docs.example.com" (no scheme, port,
    /// or path). At most 64 entries; an empty list is rejected (omit the field instead).
    ...
    /// <summary>
    /// Maximum number of tokens of fetched text content to include in context per
    /// call. Does not apply to binary content such as PDFs.
    /// </summary>
    ...
    public BetaManagedAgentsWebFetchToolConfigParamsPermissionPolicy? PermissionPolicy
    {
        ...
        {
            this._rawData.Freeze();
            return this._rawData.GetNullableClass<BetaManagedAgentsWebFetchToolConfigParamsPermissionPolicy>(
                "permission_policy"
            );
        }
    ...
    public ApiEnum<string, BetaManagedAgentsWebFetchToolConfigParamsType>? Type
    {
        get
        {
            this._rawData.Freeze();
...
#pragma warning disable CS8618
    [SetsRequiredMembers]
    public BetaManagedAgentsWebFetchToolConfigParams(
        BetaManagedAgentsWebFetchToolConfigParams betaManagedAgentsWebFetchToolConfigParams
    )
        : base(betaManagedAgentsWebFetchToolConfigParams) { }
...
#pragma warning disable CS8618
    [SetsRequiredMembers]
    BetaManagedAgentsWebFetchToolConfigParams(FrozenDictionary<string, JsonElement> rawData)
    {
        this._rawData = new(rawData);
    }
#pragma warning restore CS8618

    /// <inheritdoc cref="BetaManagedAgentsWebFetchToolConfigParamsFromRaw.FromRawUnchecked"/>
    public static BetaManagedAgentsWebFetchToolConfigParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawData
    )
    {
...
class BetaManagedAgentsWebFetchToolConfigParamsFromRaw
    : IFromRawJson<BetaManagedAgentsWebFetchToolConfigParams>
{
    /// <inheritdoc/>
    public BetaManagedAgentsWebFetchToolConfigParams FromRawUnchecked(
        IReadOnlyDictionary<string, JsonElement> rawData
    ) => BetaManagedAgentsWebFetchToolConfigParams.FromRawUnchecked(rawData);
}
...
public record class BetaManagedAgentsWebFetchToolConfigParamsPermissionPolicy : ModelBase
{
    ...
    public BetaManagedAgentsWebFetchToolConfigParamsPermissionPolicy(
    ...
    public BetaManagedAgentsWebFetchToolConfigParamsPermissionPolicy(JsonElement element)
    {
        this._element = element;
    }
        ...
        switch (this.Value)
        {
            ...
            default:
                throw new AnthropicInvalidDataException(
                    "Data did not match any variant of BetaManagedAgentsWebFetchToolConfigParamsPermissionPolicy"
                );
        }
    }
    ...
    public static implicit operator BetaManagedAgentsWebFetchToolConfigParamsPermissionPolicy(
        BetaManagedAgentsAlwaysAllowPolicy value
    ) => new(value);
    ...
    public virtual bool Equals(BetaManagedAgentsWebFetchToolConfigParamsPermissionPolicy? other) =>
        other != null
...
sealed class BetaManagedAgentsWebFetchToolConfigParamsPermissionPolicyConverter
    : JsonConverter<BetaManagedAgentsWebFetchToolConfigParamsPermissionPolicy?>
{
    public override BetaManagedAgentsWebFetchToolConfigParamsPermissionPolicy? Read(
    ...
    )
    {
        ...
        switch (type)
        {
            ...
            default:
            {
                return new BetaManagedAgentsWebFetchToolConfigParamsPermissionPolicy(element);
            }
        }
    }
...
[JsonConverter(typeof(BetaManagedAgentsWebFetchToolConfigParamsTypeConverter))]
public enum BetaManagedAgentsWebFetchToolConfigParamsType
{
    WebFetch,
}

sealed class BetaManagedAgentsWebFetchToolConfigParamsTypeConverter
    : JsonConverter<BetaManagedAgentsWebFetchToolConfigParamsType>
{
    public override BetaManagedAgentsWebFetchToolConfigParamsType Read(
        ref Utf8JsonReader reader,
        System::Type typeToConvert,
        JsonSerializerOptions options
    )
    {
        ...
        {
            "web_fetch" => BetaManagedAgentsWebFetchToolConfigParamsType.WebFetch,
            _ => (BetaManagedAgentsWebFetchToolConfigParamsType)(-1),
        };
    }

    public override void Write(
        Utf8JsonWriter writer,
        BetaManagedAgentsWebFetchToolConfigParamsType value,
        JsonSerializerOptions options
    )
    {
        JsonSerializer.Serialize(
            ...
            value switch
            {
                BetaManagedAgentsWebFetchToolConfigParamsType.WebFetch => "web_fetch",
                _ => throw new AnthropicInvalidDataException(
