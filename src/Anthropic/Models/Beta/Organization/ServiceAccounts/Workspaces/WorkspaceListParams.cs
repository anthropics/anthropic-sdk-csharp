...
///
/// <para>Each entry includes the service account's `workspace_role` in that workspace.
/// Use `limit` and the `next_page` cursor to paginate. When the service account
/// has no explicit default-workspace membership, the implicit (`implicit: true`)
/// membership is returned as the first entry on the first page; with `limit=1` the
/// first page may return up to 2 entries (the implicit entry plus one explicit membership)
/// so a pagination cursor can be derived. Memberships are returned only while the
/// service account is active. Without a `page` cursor, an archived service account
/// returns an empty list. A `page` cursor that does not match an active membership
/// returns a 400 invalid-request error. A cursor stops matching when the membership
/// is removed, the workspace is deleted, or the service account is archived. Restart
/// pagination from the first page to recover.</para>
///
...
#pragma warning disable CS8618
    ...
    {
        this.ServiceAccountID = workspaceListParams.ServiceAccountID;
    }
