...
public record class RateLimitListParams : ParamsBase
{
    ...
    /// <summary>
    /// Filter to the single entry containing this model. Accepts full model names
    /// and aliases. Returns 404 if the model is not found or has no rate limits for
    /// this organization.
    /// </summary>
    public string? Model
    {
        ...
        {
            this._rawQueryData.Freeze();
            return this._rawQueryData.GetNullableClass<string>("model");
        }
        init { this._rawQueryData.Set("model", value); }
    }
...
#pragma warning restore CS8618
    ...
    public override Uri Url(ClientOptions options)
    {
        ...
        return new UriBuilder(
            options.BaseUrl.ToString().TrimEnd('/') + "/v1/organizations/rate_limits"
        )
        {
