...
var vault = await client.Beta.Vaults.Create(new VaultCreateParams
{
    DisplayName = "comprehensive-example-vault",
});
Console.WriteLine($"Created vault: {vault.ID}");

var credential = await client.Beta.Vaults.Credentials.Create(vault.ID, new CredentialCreateParams
{
...
var skill = await client.Beta.Skills.Create(new SkillCreateParams
{
...
var agentV1 = await client.Beta.Agents.Create(new AgentCreateParams
{
...
var agent = await client.Beta.Agents.Update(agentV1.ID, new AgentUpdateParams
{
...
await foreach (var streamEvent in client.Beta.Sessions.Events.StreamStreaming(session.ID))
{
    ...
    if (streamEvent.TryPickAgentCustomToolUseEvent(out var toolUse) && toolUse.Name == "get_weather")
    {
        await client.Beta.Sessions.Events.Send(session.ID, new EventSendParams
        {
            Events =
    ...
    if (streamEvent.TryPickSessionStatusIdleEvent(out var idleEvent)
